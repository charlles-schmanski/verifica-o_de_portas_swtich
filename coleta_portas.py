...
def configurar_logging(verbose: bool = False) -> None:
    ...
    arquivo_log = pasta_logs / f"coleta_{data_log}.log"
...
def salvar_historico(
    resultados: list[PortaResultado],
    pasta_historico: str,
) -> None:
    if not resultados:
        LOG.warning(
            "Nenhum resultado para salvar no historico."
        )
        return
    ...
    arquivo_csv = (
        pasta_data
        / f"portas_{hora_arquivo}.csv"
    )

    arquivo_json = (
        pasta_data
        / f"portas_{hora_arquivo}.json"
    )
    ...
    LOG.info(
        "Historico CSV salvo em: %s",
        arquivo_csv,
    )

    LOG.info(
        "Historico JSON salvo em: %s",
        arquivo_json,
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        # Mantem os arquivos mais recentes para consulta e dashboard.
        salvar_csv(todos_resultados, args.csv_saida)
        salvar_json(todos_resultados, args.json_saida)

        # Preserva uma copia da coleta em historico/AAAA-MM-DD/.
        salvar_historico(todos_resultados, PASTA_HISTORICO)
    except Exception as e:
    ...
    LOG.info("Logs diarios gerados na pasta: %s", PASTA_LOGS)
